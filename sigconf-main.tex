...
The conceptual social media framework from Carr and Hayes \cite{Carr2015} explains three pillars of why mindless scrolling easily occurs: disentrainment (time-asynchronous communication), persistence (the part where users are always aware), and perceived interactivity. In short, the combination created affects even passive users, leading them to consume content endlessly and without purpose. This theoretical understanding is complemented by the neuropsychological explanation from Sinha et. al. \cite{Sinha2023}, which focused on how the infinite scroll design triggers a dopamine reward system and creates an automatic habit. This discovery provides further evidence that the infinite scroll design triggers users to keep engaging in mindless scrolling repeatedly, which affects their brain sensitivity due to dopamine tolerance, causing them to need more intense stimulation to achieve the same sensation. The resulting cognitive fatigue negatively affects daily well-being and leads to feelings of guilt about being unproductive. This guilt shows a continuous effect on both memory and attention decline, which happens not only situationally but also reflects cognitive fatigue due to continuous partial attention reinforced by platform design \cite{deSegovia2024}. This finding urges experts to test the roles of self-control and situational context. Participants with lower self-control show a strong connection between scrolling and goal conflict, whereas work context worsens the same effects. This shows that mindless scrolling is not just a bad habit but also an interactive phenomenon between system design, psychological condition, and user activity context.
