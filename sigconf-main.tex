...
The study used a within-subjects design and each subject experienced both the calm and the mindless-scrolling conditions. Two sets of sessions were prepared, with each session containing both conditions but in a different order. The sessions in set A started with the calm condition followed by the mindless-scrolling condition. The sessions with set B started with the mindless-scrolling condition followed by the calm condition. The participants were equally and randomly assigned to one of the two sets. In order to mitigate the carry-over effects between the two conditions, subjects were given five minute breaks before moving on from the first condition to the second condition. 
